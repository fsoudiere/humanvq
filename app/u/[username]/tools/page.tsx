...
export default function ToolsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Filter out null resources and only AI tools (not courses)
  ...
  // Group by status
  const payingTools = filteredResources.filter((i: any) => i.status === 'added_paid')
  const freeTools = filteredResources.filter((i: any) => i.status === 'added_free')
  const wishlistTools = filteredResources.filter((i: any) => i.status === 'wishlisted')
      ...
      <main className="mx-auto max-w-6xl px-6 pt-8 pb-16">
        ...
        {filteredResources.length === 0 ? (
          <Card className="p-12 text-center">
            <CardContent>
              <p className="text-zinc-500">No tools found matching your filters.</p>
            </CardContent>
          </Card>
        ) : (
          <TierSection
