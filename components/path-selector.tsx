...
export default function PathSelector({ username }: { username: string }) {
    ...
    }, [username, pathname]) // Add pathname dependency to refetch when slug changes
    ...
    // Find current path by matching slug first, then fallback to ID
    // This ensures we use the updated slug after title changes
    const currentPath = currentPathSlug
        ? paths.find(p => {
            // Prioritize slug match (most up-to-date)
            if (p.slug && p.slug === currentPathSlug) return true
            // Fallback to ID match (for paths with null slug)
            if (p.id === currentPathSlug) return true
            return false
        })
        : null
                ...
                <>
                    ...
                    <div className="absolute top-full left-0 mt-1 z-50 min-w-[250px] max-w-[300px] bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-md shadow-lg py-1">
                        {paths.map((path) => {
                            ...
                            // Always prefer slug over ID for URL (slug is more user-friendly)
                            const pathUrl = `/u/${username}/${path.slug || path.id}`
                            // Check if this is the active path by matching slug first, then ID
                            const isActive = path.slug
                                ? path.slug === currentPathSlug
                                : path.id === currentPathSlug
                            ...
                            return (
                                <Link
                                    ...
                                    href={pathUrl}
                                    onClick={() => setIsOpen(false)}
                                    className={`block px-4 py-2 text-sm hover:bg-zinc-100 dark:hover:bg-zinc-800 ${isActive ? "bg-zinc-100 dark:bg-zinc-800 font-medium" : ""
                                        }`}
                                >
